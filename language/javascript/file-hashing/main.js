...
/**
 * @param {string} fileUrl - file url to fetch.
 * @return {Map<any, any>} - file's sha1 and md5.
 */
const fileHash = async function (fileUrl) {
    ...
    const sha1 = CryptoJS.SHA1(text)
    const md5 = CryptoJS.MD5(text)
    return [CryptoJS.enc.Hex.stringify(sha1), CryptoJS.enc.Hex.stringify(md5)]
}
...
const main = async function (args) {
    ...
    const resultArray = await Promise.all([fileHash(args.get("file_url")).catch(e => {
        console.log("There has been a problem with fetching file: " + e.message);
    })])
    output.set("sha1", resultArray[0][0])
    output.set("md5", resultArray[0][1])
    return output
}
