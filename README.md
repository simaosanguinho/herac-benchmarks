...
This repository includes a number of benchmarks used to test and evaluate Hydra and Argo in general. The repository is split into several directories:
...
The repository currently maintains a number of benchmarks. Benchmarks are implemented in two variants: OpenWhisk (cr) and Hydra (gv):

- (hydra and OpenWhisk) Java:
...
- (hydra and OpenWhisk) JavaScript:
...
After building a benchmark, it is possible to run it on Hydra or OpenWhisk runtimes:
- running the HelloWorld function on a Hydra container: `scripts/benchmark-hydra.sh container gv_java_hw test 1`
