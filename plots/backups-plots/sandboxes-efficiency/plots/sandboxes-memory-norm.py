...
ax.bar(x + 2*width, process_mem_avg.values(),   yerr=process_mem_std.values(),   width=width, hatch='.',  label='SAND/SOCK',  alpha=0.75)
ax.bar(x + 3*width, photons_mem_avg.values(),   yerr=photons_mem_std.values(),   width=width, hatch=',',  label='Photons',    alpha=0.75)
ax.bar(x + 4*width, openwhisk_mem_avg.values(), yerr=openwhisk_mem_std.values(), width=width, hatch='-',  label='OpenWhisk',  alpha=0.75)
